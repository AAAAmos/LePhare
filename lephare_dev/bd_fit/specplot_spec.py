...
for k in range(nspec):
    ...
    #             '$T_{eff}$ = '+teff + 'K,  g = '+grav+'cm/s$^2$,  Metallicity = '+metal, 
    #             size=18)
    ...
    model_name = ["Galaxy model", "Galaxy model 2", "Galaxy model (FIR)", "Galaxy model (Stochastic)", "QSO model",
                "Sonora-Bobcat model"]
    ...
    for im in range(6):
        ...
        # ax1.plot(lg[im],mg[im],color=col_lst[im], label=models_info[im][0]+' Chi$^2$:'+str(float(models_info[im][8])))  #plot the SED
        ax1.plot(lg[im],mg[im],color=col_lst[im],
                label=MODname[int(models_info[im][2])-1]+' spectra'+', $\\chi^2$='+str(float(models_info[im][8])))  #plot the SED
        # del models_info[im][6:8] #do not print z_inf and z_sup
        # del models_info[im][-1]  #nor sSFR 
        # print(models_info[im]) 
        # info1=('  '.join(['%.3f']*len(models_info[im][2, 4, 8]))) % tuple([float(j) for j in models_info[im][2, 4, 8]])
        ...
        #     info2=',  '+info2+'.'
        # else: info2='.'
        # infol=models_info[im][0] + ': (' + ' ' .join(models_info[im][2:5]) + ')  ' + info1
        
        # infol = models_info[im][0]+'\n'+str(float(models_info[im][8]))+'\n'
        # # infol = models_info[im][0]+'\n'+str(float(models_info[im][2]))+'\n'+str(float(models_info[im][8]))+'\n'
        # if im==5:
        #     infol = infol + MODname[int(models_info[im][2])-1]
        # else: 
        #     infol = infol + 'nan'
            
        infol = Typename[int(models_info[im][2])-1] + '\n' + MODname[int(models_info[im][2])-1]
        
        plt.figtext(0.2+0.15*iml,0.73,infol,color=col_lst[im],size=12) #print the rest
